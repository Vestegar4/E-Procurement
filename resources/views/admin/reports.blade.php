...
@section('content')
    <div class="mb-4">
        <h4 class="fw-bold mb-1" style="color: var(--color-text-main); letter-spacing: -0.01em;">Pusat Unduhan Laporan &
            Grafik Analisis</h4>
        <p class="mb-0 fw-medium" style="color: var(--color-text-muted);">Pantau statistik realisasi keuangan sistem dan unduh
            data mentah pengadaan</p>
    </div>
    ...
    <div class="row g-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
                    <div>
                        <h5 class="fw-bold mb-1" style="color: var(--color-text-main);">
                            <i class="fa-solid fa-chart-line me-2" style="color: var(--color-accent-bright);"></i>Tren
                            Realisasi Anggaran Pengadaan
                        </h5>
                        <p class="text-muted small mb-0">Grafik akumulasi volume nilai transaksi Purchase Order (PO) per
                            bulan berdasarkan tahun</p>
                    </div>

                    <div class="d-flex align-items-center gap-3">
                        <form action="{{ route('admin.reports') }}" method="GET" class="d-flex" id="yearFilterForm">
                            <select name="year" class="form-select fw-bold shadow-sm"
                                style="border-radius: 8px; border-color: var(--color-border); color: var(--color-text-main); min-width: 120px;"
                        ...
                        <span class="badge rounded-pill px-3 py-2 fw-bold"
                            style="background-color: var(--color-success-bg); color: var(--color-success-text); border: 1px solid var(--color-success-border);">
        ...
        {{-- Card 1: Rekapitulasi --}}
        <div class="col-md-6">
            <div class="card card-custom p-5 text-center border-0 shadow-sm d-flex flex-column align-items-center"
                style="background: var(--color-white); border-radius: var(--radius-card, 16px); height: 100%;">
                <div class="rounded-circle d-flex align-items-center justify-content-center mb-4"
                    style="width: 80px; height: 80px; background: var(--color-surface); border: 2px solid var(--color-border);">
                ...
                <p class="text-muted mb-4 px-3">Ekstrak seluruh daftar riwayat tender yang pernah dibuat ke dalam format
                    CSV.</p>
                <a href="{{ route('admin.reports.download', 'procurement') }}" class="btn btn-primary-action w-100 mt-auto">
        ...
        <div class="col-md-6">
            <div class="card card-custom p-5 text-center border-0 shadow-sm d-flex flex-column align-items-center"
                ...
                <p class="text-muted mb-4 px-3">Unduh basis rekap profil mitra kerja berstatus terverifikasi dan riwayat PO.
                </p>
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    let val = context.raw;
                                    return 'Total Nilai: Rp ' + new Intl.NumberFormat('id-ID').format(
                                        val);
                                }
                            }
                        }
