                ...
                <div class="row g-2 align-items-center">
                    ...
                    <div class="col-md-5">
                        <div class="input-group">
                            ...
                            <input type="text" name="search" class="form-control border-start-0 auth-input px-0"
                                placeholder="Ketik ID PO lalu tekan enter..." value="{{ request('search') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-5">
                        <select name="status" class="form-select auth-input" onchange="this.form.submit()"
                            ...
                            <option value="draft" {{ request('status') == 'draft' ? 'selected' : '' }}>Draft (Konsep
                                Tersimpan)</option>
                            <option value="issued" {{ request('status') == 'issued' ? 'selected' : '' }}>Issued (Menunggu
                                Pengiriman)</option>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead
                            style="background-color: var(--color-surface); color: var(--color-text-main); font-weight: 700;">
                            <tr>
                        ...
                        <tbody>
                            @foreach ($purchaseOrders as $po)
                                <tr style="border-bottom: 1px solid rgba(0,0,0,0.05);">
                                    ...
                                    <td class="fw-semibold" style="color: var(--color-text-main);">
                                        {{ $po->tender->title ?? 'N/A' }}</td>
                                    <td style="color: var(--color-text-muted); font-weight: 500;">
                                        {{ $po->vendor->company_name ?? ($po->vendor->name ?? 'N/A') }}</td>
                                    <td class="fw-bold" style="color: var(--color-accent);">Rp
                                        {{ number_format($po->total_amount, 0, ',', '.') }}</td>

                                    <td>
                                        @if (strtolower($po->status) == 'draft')
                                            <span class="badge rounded-pill px-3 py-2 fw-bold text-dark shadow-sm"
                                                style="background-color: #d1d5db; border: 1.5px solid #a6a9ae; font-size: 0.75rem;">
                                        ...
                                        @elseif(strtolower($po->status) == 'issued')
                                            <span class="badge rounded-pill px-3 py-2 fw-bold text-white shadow-sm"
                                                style="background-color: #41ff00; font-size: 0.75rem;">
                                        ...
                                        @elseif(strtolower($po->status) == 'cancelled')
                                            <span class="badge rounded-pill px-3 py-2 fw-bold shadow-sm"
                                                style="background-color: var(--color-danger-bg); color: var(--color-danger-text); border: 1px solid var(--color-danger-border); font-size: 0.75rem;">
                                                <i class="fa-solid fa-circle-xmark me-1"></i> CANCELLED
                                            </span>
                                        @elseif(strtolower($po->status) == 'completed')
                                            <span class="badge rounded-pill px-3 py-2 fw-bold shadow-sm"
                                                style="background-color: var(--color-success-bg); color: var(--color-success-text); border: 1px solid var(--color-success-border); font-size: 0.75rem;">
                                                <i class="fa-solid fa-check-double me-1"></i> COMPLETED
                                            </span>
                                        @else
                                            <span
                                                class="badge rounded-pill px-3 py-2 fw-bold bg-secondary text-white shadow-sm"
                                        ...
                                        {{-- Form Ubah Status --}}
                                        <form action="{{ route('admin.purchase-orders.update-status', $po->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <select name="status" class="form-select form-select-sm text-center shadow-sm"
                                                onchange="this.form.submit()"
                                                style="border-radius: 6px; cursor: pointer; font-size: 0.75rem; font-weight: 600;">
                                                <option value="draft"
                                                    {{ strtolower($po->status) == 'draft' ? 'selected' : '' }}>Ubah ke
                                                    Draft</option>
                                                <option value="issued"
                                                    {{ strtolower($po->status) == 'issued' ? 'selected' : '' }}>Issued PO
                                                </option>
                                                <option value="cancelled"
                                                    {{ strtolower($po->status) == 'cancelled' ? 'selected' : '' }}>Cancel
                                                    PO</option>
                                                <option value="completed"
                                                    {{ strtolower($po->status) == 'completed' ? 'selected' : '' }}>Complete
                                                    PO</option>
                                            </select>
                                        </form>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <a href="/admin/purchase-orders/{{ $po->id }}/export-pdf" target="_blank"
                                            class="btn btn-outline-danger btn-sm mb-2 w-100 fw-bold"
    ...
    @else
        <div class="card card-custom border-0 shadow-sm"
            ...
            <div class="card-body p-5 text-center">
                ...
                <p class="fw-bold text-muted mb-0">Belum ada dokumen Purchase Order yang terbit atau sesuai pencarian
                    filter.</p>
            </div>
        </div>
    @endif
